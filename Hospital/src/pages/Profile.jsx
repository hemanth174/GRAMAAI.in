...
import { User, Mail, Phone, Building, Save, Edit, Loader2, CheckCircle, Lock, Shield, Bell, Trash2, X } from 'lucide-react';
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        const storedHospitalName = storedUser.hospitalName || storedUser.hospital_name;
        ...
        <div className="flex items-center justify-between">
          ...
          {!isEditing ? (
            <button
              ...
              className="flex items-center justify-center w-10 h-10 rounded-full bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-110 active:scale-95"
            >
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex items-center justify-center w-10 h-10 rounded-full bg-gray-200 text-gray-700 shadow-md hover:shadow-lg transition-all duration-300 hover:scale-110 active:scale-95"
              >
                <X className="w-5 h-5" />
              </button>
              <button
                onClick={handleSave}
                disabled={isSaving}
                className="flex items-center justify-center w-10 h-10 rounded-full bg-gradient-to-r from-green-600 to-emerald-600 text-white shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-110 active:scale-95 disabled:opacity-50"
              >
            ...
            <div className="flex flex-col sm:flex-row items-center sm:items-end gap-4 -mt-12 sm:-mt-16 md:-mt-20">
              <div className="relative flex-shrink-0">
                <div className="w-24 h-24 sm:w-32 sm:h-32 md:w-40 md:h-40 bg-gradient-to-br from-blue-500 via-indigo-500 to-purple-600 rounded-full flex items-center justify-center ring-4 sm:ring-6 ring-white shadow-2xl">
                  <span className="text-3xl sm:text-4xl md:text-5xl font-bold text-white">
                    {(currentUser.full_name || currentUser.fullName || 'H').charAt(0)}
                  </span>
                </div>
              ...
              <div className="flex-1 text-center sm:text-left sm:mb-4">
                <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-900">
                  {currentUser.full_name || currentUser.fullName || 'Hospital Administrator'}
                </h2>
                <p className="text-sm sm:text-base md:text-lg text-gray-600 mt-1 flex items-center justify-center sm:justify-start gap-2">
                  ...
                  {currentUser.hospital_name || currentUser.hospitalName || 'Hospital Name'}
                </p>
