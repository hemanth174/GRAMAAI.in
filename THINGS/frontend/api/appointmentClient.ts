...
// Connects patient portal to the shared hospital backend
...
export async function checkBackendConnection(): Promise<boolean> {
  try {
    const response = await fetch(`${API_BASE_URL}/api/appointments`, { method: 'HEAD' });
    return response.ok || response.status === 405; // HEAD might not be allowed, but server is up
  } catch (error) {
